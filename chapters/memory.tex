...
% To fill this gap, we develop and empirically validate a computational model that characterizes the role of metamemory during recall. 
% In contrast to previous work that identified empirical associations between metamemory judgments and behavior patterns such as search times (???), our model gives predictions on the optimal behavior we should expect if metamemory indeed plays a role during effective control. 
% Our model is also the first (to our knowledge) in providing a computational explicit instantiation of the classic theory of metamemory proposed by Nelson and Narens in the context of memory recall.

% Nelson and Narens (1990) proposed that the primary function of a metacognitive system is to allow effective control of ongoing cognition

% ALTERNATIVE: ...attempts to explain how both metamemory judgments and control could emerge from a single metamemory process.

% We are aware of only one such model, one of memory maintenance \citep{suchow2016deciding}.
...
% However, this approach typically abstracts away from the dynamics of cognition, summarizing an extended cognitive process as a single maximization or application of Bayes' rule. It is thus unclear how the approach could be applied to a phenomenon such as metamemory that is inherently dynamic, involving a cyclic interplay between two cognitive processes (Figure~\ref{fig:nelson}). Specifically,
% However, this approach typically abstracts away from mechanism, fosucing solely on the output of the cognitive system \citep{anderson1991adaptive,marr1982vision}. It is unclear how such an approach could be applied to a phenomenon such as metamemory that is inherently about cognitive processes, i.e. how they are monitored and controlled. To address this challenge, we draw on the framework of resource-rational analysis, which characterizes cognitive processes as optimal subject to cognitive costs (\citealp{griffiths2015rational,lieder2020resourcerational}; c.f. \citealp{sims2003implications,howes2009rational,lewis2014computational,gershman2015computational}).
The remainder of this paper is organized as follows. We begin by reviewing empirical work on meta-level control of memory, focusing on the control of recall. Next, we define an optimal model of meta-level control in memory recall and characterize its predictions. Notably, the model predicts that uncuccessful memory searches will be longer when the target memory is (judged to be) stronger, consistent with the findings of \citet{costermans1992confidence}. Next, we describe a cued-recall experiment that conceptually replicates and extends those findings. We confirm all key qualitative predictions of the model and establish moderate quantitative fit. Our second experiment extends the first by allowing participants to choose between two possible recall targets. This introduces a more complex meta-level control problem of selecting which memory to search for at each moment. Using a keypress-contingent display, we compare the timecourse of attention to each cue with the optimal model's search predictions and again achieve a strong qualitative and moderate quantitative fit. We conclude by discussing implications of the results for metamemory and metacognition research more generally, and identifying interesting directions for further research.
...
% Fred: I think this is too much detail for the introduction

% Critical to addressing above two questions is our development of a novel computational model of metamemory. `` `Metacognition' is simultaneously a topic of interest in its own right and a bridge between areas, e.g. between decision-making and memory'' \citep{nelson1994investigate}. Our approach expands Nelson classic formulation of meta-level and object-level (Figure~\ref{fig:nelson}; \citealpnelson{}), while introducing the state-of-art modeling approaches from the decision-making and memory literature. In particular, we set the meta-level as a rational decision maker who optimizes the goal (e.g. correctly retrieve the answer using minimal time) and set the object-level being a memory model that captures the retrieval process of evidence accumulation. To let the two levels interact, meta-level monitors the progress at the object-level, and uses that information to exert control on the object-level regarding how to best direct retrieval efforts. We also construct an alternative model without a meta-level process: participants continue memory retrieval efforts irregardless of their knowledge of their memory states. This setup is equivalent to standard memory models designated to capture general patterns memory recall but not aimed to capture the role of metamemory (\citealp{ratcliff2002estimating}; \citealp{sederberg2008context}).

% To demonstrate that human behavior in directing memory retrieval efforts is consistent with one that optimally utilizes metamemory, in the Experiment 1, we replicated the findings of \citet{costermans1992confidence} where stronger ratings of FOKs are associated with longer search times ... We showed that participants' stopping behavior in memory search was consistent with predictions from the model that contains meta-level control.

% In addition to using a standard paradigm from metamemory literature to examine search times in Experiment 1, we designed a novel paradigm in Experiment 2 to test the role of metamemory in a more dynamic task where people need to direct retrieval efforts between two competing memories. Instead of being probed to answer one cue-recall question (given an image as the cue), participants were presented with two images on each trial and were instructed to recall the word associated with either of them. Participants can flip between the two images through keypressing and direct their retrieval efforts accordingly ... This setup has a few advantages compared with Experiment 1. First, we can collect richer behavioral data throughout the memory retrieval process where participants continuously make decisions in flipping between two images. Second, assuming that the same metamemory control process applies, we can test if model parameters in Experiment 1 generalize to the more complex task in Experiment 2, which provides a strong test on ...
...
% Similarly, people are slower to give up on questions about more familiar topics, or whose key terms had been primed \citep{reder1988strategic}. Together, these findings suggest that people allocate more resources to searching for memories that they feel they are more likely to recall.
...
% In fact, there are other evidence suggesting that metamemory judgments are highly inferential process. Different kinds of retrieval failures affect the feeling of knowing: participants report having a greater judgment for items to which they were informed that they made an incorrect recall than items they had omitted making a recall response \citep{krinsky1985feeling}. 
% - Models of metamemory: BIM (hu2021bayesian) and SDRM (jang2012stochastic), based on learning experience, more JOL than FOK
% - metacognition in decision-making: flemming, sumerfield, drugowitsch, callaway
...
% To that end, we will assume a simple abstraction of the (object-level) memory search process, and assume that the meta-level process is perfectly optimal with respect to that process. 
...
% Left to its own devices, the object-level process would continue attempting to recall an out-of-reach memory (e.g. one with $v < 0$) indefinitely.
...
% [Note: Qiong has proposed putting the assumptions first, but I think it is clearer to do concrete then abstract.]
...
%
% $p(v \mid z_t, t) = \mNormal{v; \mu_t, \sigma_t^2}$ with
% \begin{equation}
% \begin{gathered}
...
% rapid negative progress provides strong evidence that the strength of the memory is low.

% Note that, while this positive correlation between strength and stopping time holds in general, the rare cases in which the model gives up on a strong memory actually tend to have fast response times because this situation can only occur if the recall progress is much slower than it would be in expectation, which can only occur for short periods of time (c.f. the ``fast errors'' phenomenon \tocite).
...
% Furthermore, these parameters led the model to predict that response time was \emph{mostly} driven by the non-decision time, thus predicting almost no effect of pretest accuracy on response time even on correct trials. Thus, in order to 
...
% Although the qualitative predictions are largely insentive to these parameters (as shown in Appendix~XXX), they must be tuned to produce predictions in a similar range to the human data. We 
...
% ``In this final test round, you will have to guess the words like before, but we've raised the stakes. This time, you'll earn 3 cents for every correct response. But you'll \emph{lose} one cent for every \emph{incorrect} response. You can \emph{skip} a round by pressing enter without typing anything in the text box. \emph{There is no penalty for skipping.} We've also raised the speed bonus to a tenth of a cent for each second left on the timer when you respond. And, unlike before, you will earn the bonus even if you don't give a correct response. So if you don't think you know the word, it might be best to quickly skip the trial to get the time bonus and avoid the error penalty.''.
...
%A positive relationship between estimated strength and skip times can only occur (in the model) when the policy intentionally skips memories that seem weak, a form of metacognitive control.

% \footnote{The opposite pattern is due to a selection effect. Given sufficient time, a moderate-strength memory is likely to be recalled; thus, a strong memory will only be skipped if a short stopping time was sampled.}
...
% Note that strong cues are fixated less often overall because such cues are more likely to be recalled quickly, resulting in fewer fixations per trial.
...
% However, for the most part, RL has been used to model the interaction between agents and \emph{external} environments. By using RL to model an agent's \emph{internal} environment


% This idea, that an agent's ``environment may lie, in part, within the skin of the biological organism'' is not new \citep{simon1955behavioral}.
...
% Since Anderson's pioneering work, . For example, in reconstruction tasks, people draw on prior knowledge of a stimulus category to adjust for memory imprecision \citep{huttenlocher2000categories}, drawing on more abstract categories for less familiar stimuli \citep{hemmer2009bayesian}. In free recall tasks, where the goal is to recall as many items as possible from a studied list, the optimal policy is to start from the beginning of the list and then sequentially recall forwards, providing a rational account of the often-observed primacy and forward asymmetry effects \citep{zhang2022optimal}.

% In free recall tasks, where the goal is to recall as many items as possible from a studied list, the optimal policy is to start from the beginning of the list and then sequentially recall forwards, providing a rational account of the often-observed primacy and forward asymmetry effects \citep{zhang2022optimal}. 

% In the spirit of rational analysis, various findings in the memory literature including memory distortions have been reexamined to determine if people are rational in achieving their goals \citep{huttenlocher2000categories,hemmer2009bayesian, steyvers2006probabilistic,xu2010rational, allred2016working, wilson2021rational}.
...
% Focusing on memory, resource-rational models have been used to explain the order in which people recall items in free recall \citep{zhang2022optimal}, the 
...
% The fact that the human data follows the same pattern speaks to the strength of the rational modeling approach. By assuming optimality, we can make \emph{a priori} predictions about how behavior will change as the structure of the problem changes.
...
% Participants in free recall experiments are typically given a fixed amount of time to recall as many items as possible from a studied list; thus, there is little empirical evidence about how people decide to terminate recall in this setting. 

% One interesting finding, however, is that the duration between the last successful retrieval and the termination response (``exit latency'') decreases as the total number of items recalled increased \citep{miller2012recall}. This finding 

% However, participants in free recall experiments are typically given a fixed amount of time to recall as many items as possible from a studied list; thus, little is known about how people decided to terminate recall in this setting. 
...
% FRED: This paragraph feels out of place. Perhaps we can find other places to cite zhang and lu. (We could probably leave lu out if we can't find a place).
% Other important factors to optimize memory recall performance include the decision about how to initiate the memory search and in which order to recall a sequences of items \citep{zhang2022optimal}. One may also choose to rely on external memory aids, perhaps based on an estimate of the probability of recall without aid \citep{hu2019role}. In some cases, one might decide not to search long-term memory at all and instead rely on information in working memory \citep{lu2022neural}. 
...
% We hope that by demonstrating the generality of this approach, we will encourage other researchers to apply it in yet other domains of cognition.
...
% Comparing our results to similar studies in value-based choice also revealed another interesting discrepancy between the two cases. A robust finding in the value-based case is that final fixations are shorter than non-final fixations \citep{krajbich2010visual,krajbich2011multialternative,tavares2017attentional}. This is predicted by attention-guided evidence accumulation models because the final fixation is cut short when the threshold is crossed. In contrast, we found that final fixations were \emph{longer} than non-final fixation ($B = 0.872$, 95\% CI [0.813, 0.930], $t(435.9)=29.29$, $p < .001$). The optimal model predicts this effect, and we initially interpreted this as evidence for meta-level control. However, fitting the lesioned model to the data, we found that it could capture the effect as well (discussed further in Appendix~\ref{app:final}). This underscores the importance of evaluating model predictions in comparison with the predictions of strong baselines.

% In contrast, evidence accumulation models of attention-guided decision-making
% % (with or without a metacognitive component)
% predict that final fixations will be shorter than non-final fixations, a prediction that is consistently confirmed in data \citep{krajbich2010visual,krajbich2011multialternative,tavares2017attentional}.

% % , a prediction that is sometimes supported by data.\footnote{%
% %     In value-based choice, \citet{krajbich2010visual} report little to no effect of value on fixation durations. However, in a perceptual choice task, \citet{tavares2017attentional} find a moderate effect.
% % } 
% Second, the model predicts---and again, our results confirm---that final fixations are longer than non-final fixations. In contrast, evidence accumulation models of attention-guided decision-making
% % (with or without a metacognitive component)
% predict that final fixations will be shorter than non-final fixations, a prediction that is consistently confirmed in data \citep{krajbich2010visual,krajbich2011multialternative,tavares2017attentional}. The fact that such a simple structural change can account for these major qualitative differences in the allocation of attention in value-based choice vs. memory suggests the promise of our MDP-based approach as a general framework for modeling metacognition.

% Although Nelson and Narens focused on metacognition in memory in particular, the notion of interacting object-level and meta-level processes (Figure~\ref{fig:nelson}) is likely applicable to many other forms of metacognition. Our approach of formalizing this theory as a meta-level Markov decision could likewise be applied outside of memory, and perhaps even outside of decision-making (where it has already been applied). Exploring applications of this general framework to other domains of metacognition is a broad, but potentially promising direction for future research.

% how to transition from recalling one item to another item during the search \citep{zhang2022optimal}.

% These models, together with the present work, relate broadly to the rational analysis framework that provided a methodology for understanding the purpose or function of certain cognitive processes, in our case, the role of metamemory. It specifies the goals of the system, the nature of the environment, and formally derives an optimal solution to achieving those goals in the environment (\citealp{anderson1989human, anderson1991adaptive}; see also bounded rationality: \citealp{simon1978rationality}; cognitive-bounded rational analysis: \citealp{howes2009rational}; resource-rational analysis: \citealp{lieder2020resourcerational}). It explains why certain patterns of cognitive processes occur because these patterns are useful in approximating the optimal solution of the cognitive task. In the spirit of rational analysis, various findings in the memory literature including memory distortions have been reexamined to determine if people are rational in achieving their goals \citep{huttenlocher2000categories,hemmer2009bayesian, steyvers2006probabilistic,xu2010rational, allred2016working, wilson2021rational}. 
