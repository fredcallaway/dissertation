...
\end{equation}
where $\cost(π)$ captures the opportunity cost of the resources consumed by $\pi$. Note that the value, expected utility, and cost all depend on the brain $B$ but we have supressed the subscript to keep notation simple. \citet{lieder2018bounded} presents a derivation of this cost in terms of the amount of time that different resources are allocated, but more nuanced factors related to the neural implementation of cognitive processes are likely at play as well \citep{musslick2021rationalizing}.
