...
\Chapter[Fixation patterns in simple choice reflect optimal information sampling\footnote{
  This chapter is based on the following paper:\newline\bibentry{callaway2021fixation}. We thank Ian Krajbich for his help in simulating the aDDM and Bas van Opheusden for suggesting the method for efficiently computing $\VPI$.
...
% \subsection{Sequential sampling model}\label{sec:attention-sequential}
...
Overall, \figref{fig:attention6}{} shows that our main model provides a better fit to some fixation patterns, whereas the aDDM provides a better fit to others. However, it is important to keep in mind that whereas our model provides predictions for these fixation patterns based on first principles, the predictions of the aDDM for these patterns are largely mechanistic since that model samples fixation locations and durations from the observed empirical distribution. As a result, it is not surprising that \figref{fig:attention6}{b} shows a better match between the aDDM and the data since the ``predicted'' durations are actually sampled from the observed data conditional on the first item rating.
